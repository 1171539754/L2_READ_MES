...
class MESClient:
    ...
    def fetch_furnace_time_data(self):
        try:
            ...
            # ---------- 改动：从数据库获取最近时间 ----------
            # 查询 CCMEvent.CCM_HeatStartTime 最大的文档（即最近同步过的时间）
            last_doc = self.furnace_collection.find_one(
                {"CCMEvent.CCM_HeatStartTime": {"$exists": True}},
                sort=[("CCMEvent.CCM_HeatStartTime", -1)],
                projection={"CCMEvent.CCM_HeatStartTime": 1}
            )
            if last_doc and last_doc.get("CCMEvent", {}).get("CCM_HeatStartTime"):
                last_time = last_doc["CCMEvent"]["CCM_HeatStartTime"]
            ...
            for item in data_list:
                ...
                if heat_start_time :
                    if existing is None or existing.get("HeatStartTime") != heat_start_time:
                        update_fields["CCM_HeatStartTime"] = heat_start_time
                ...
                operations.append(
                    UpdateOne(
                        {"HeatName": heat_name},
                        {"$set":
                             {"CCMEvent":update_fields,
                              "LFEvent":{
                                  "LF_process":LF_process #权宜之计，后续需要修改
                              }
                              }
                             },
                        upsert=True
